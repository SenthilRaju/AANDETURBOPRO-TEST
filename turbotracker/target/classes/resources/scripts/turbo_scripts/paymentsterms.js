...
function editData() {
	...
	$.ajax({
		...
		success: function(data) {
			createtpusage('Company-Settings','Edit Payment Terms','Info','Company-Settings,Updating Payment Terms,description:'+ $("#description").val());
...
*/
			createtpusage('Company-Settings','Save Payment Terms','Info','Company-Settings,Saving Payment Terms,description:'+ $("#description").val());
			$("#chartsOfTermsGrid").trigger("reloadGrid");
...
function deletePaymentTerms(){
	...
	if(rowId !== null){
		...
		jQuery(newDialogDiv).dialog({modal: true, width:300, height:120, title:"Confirm Delete", 
			buttons:{
				"Submit": function(){
					createtpusage('Company-Settings','Delete Payment Terms','Info','Company-Settings,Saving Shipvia Details,description:'+ $("#description").val());
					deleteData();
					jQuery(this).dialog("close");
					$("#chartsOfTermsGrid").trigger("reloadGrid");
				},
				Cancel: function ()	{jQuery(this).dialog("close"); $("#chartsOfTermsGrid").trigger("reloadGrid");} } }).dialog("open");
		return true;
	}else{
...
function deleteData() {
	...
	$.ajax({
		...
		success: function(data) {
			var validateMsg = "<b style='color:Green;'>Payment Terms are Deleted Successfully.</b>";	
			$('#paymentTermsMsg').html(validateMsg);
			setTimeout(function(){
			$('#paymentTermsMsg').html("");						
			},3000);
			...
			jQuery(newDialogDiv).dialog({modal: true, width:300, height:150, title:"Success.", 
									buttons: [{height:35,text: "OK",click: function() {  $(this).dialog("close"); 
									document.getElementById("paymentTermsFromID").reset();$("#chartsOfTermsGrid").trigger("reloadGrid"); }}]}).dialog("open");
			$('#settings_tabs').tabs({ selected: 1 });
			$('#settings_tabs').tabs({ active: 1 });*/
		}
	});
