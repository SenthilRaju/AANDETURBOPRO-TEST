jQuery(document).ready(function(){
	...
	$("#jobsGrid").jqGrid({
		...
		colModel :[
            {name:'joMasterId', index:'joMasterId', align:'left', width:40, editable:true,hidden:true, edittype:'text',
		...
		ondblClickRow: function(rowId) {
			...
			var aQryStr = "jobNumber=" + jobNumber + "&jobName=" + urijobname + "&jobCustomer=" + uricusname+"&joMasterID="+joMasterID;;
...
$( "#searchJob" ).autocomplete({ minLength: 3,
	select: function (event, ui) {
			...
			$.ajax({
				...
				mType: "GET",
				data : {'jobnumber': jobNumber, 'jobname': jobName,'joMasterID':id},
				success: function(data){
					...
					var urijobname=encodeBigurl(jobName);
					var uricusname=encodeBigurl(jobCustomer);
          		window.location.href = "./jobflow?token=view&jobNumber="+jobNumber+"&jobName="+urijobname+"&jobCustomer="+uricusname+"&jobStatus="+status+"&joMasterID="+id;; 
			},
			error: function(Xhr) {
			}
		});
