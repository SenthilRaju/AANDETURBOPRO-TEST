...
function poreleasedialog() {
	 createtpusage('job-Submittal Tab','PO Release','Info','job-Submittal Tab,PO Release Button,Job Number:'+ $('input:text[name=jobHeader_JobNumber_name]').val());
			...
			success: function(data) {
				...
				if(!validNewRelease) {
					...
					if(jQuery("#submittalList").getGridParam("records") === 0){
						...
						console.log("Manu---->"+aManufacturerId);
						//if(aManufacturerId == )
						...
						console.log("poreleasedialog()--->"+aReleaseFormValues);
						$.ajax({
...
function soreleasedialog() {
	 createtpusage('job-Submittal Tab','SO Release','Info','job-Submittal Tab,SO Release,Job Number:'+ $('input:text[name=jobHeader_JobNumber_name]').val());
	var jobStatus = getUrlVars()["jobStatus"];
	if(jobStatus === "Closed" || jobStatus === "Bid" || jobStatus === "Planning" || jobStatus === "Budget" || jobStatus === "Quote" ||
			jobStatus === "Submitted" ||  jobStatus === "Closed" || jobStatus === "Lost" ||
			jobStatus === "Abandoned" || jobStatus === "Rejected" || jobStatus === "Over Budge"){
...
function submittaldialog() {
	 createtpusage('job-Submittal Tab','Launch Submittal','Info','job-Submittal Tab,Launch Submittal');
	 var optionTag = '';
...
function submittal_save() {
	...
	$.ajax({
		...
		success: function(data) {
			createtpusage('job-Submittal Tab','Save Button','Info','Job,Submittal Tab,Saving Submittal,JobNumber:'+jobNumber);
			var errorText = "Submittal Details Successfully Updated.";
			var validateMsg = "<b style='color:Green; align:right;'>"+errorText+"</b>";	
			$("#quoteSubmittal").css("display", "block");
			$('#quoteSubmittal').html(validateMsg);
			setTimeout(function(){
			$('#quoteSubmittal').html("");						
			},2000);
			return true;
...
function printSubmittalCover(){
    createtpusage('job-Submittal Tab','Print Cover','Info','job-Submittal Tab,Print Cover Button,Job Number:'+ $('input:text[name=jobHeader_JobNumber_name]').val());
	var grid = $("#submittalList");
	var rowId = grid.jqGrid('getGridParam', 'selrow');
