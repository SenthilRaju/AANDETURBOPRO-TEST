	...
	<div  style="background-color: #FAFAFA">
		...
		<div id="addvendor">
			<div class="tabs_main" style="padding-left: 0px;width:1150px;margin:0 auto; background-color: #FAFAFA">
				<ul>
					<li><a href="vendorContacts">Contacts</a></li>
					<li><a href="rolodex/rolodexjournal">Journal</a></li>
					<li id="vendor"><a href="rolodexforms/vendorform">Financial</a></li><!-- Vendor financial information -->
					...
					<li style="float: right; padding-right: 10px; padding-top:3px;">
						<label id="customerName"  style="color: white;vertical-align: middle;" onclick="hyperLink()">${requestScope.name}</label>
	...
	<input type="text" id="CustomerCategory5hidden" name="CustomerCategory5hidden" value="${requestScope.CustomerCategory5}">
		...
		<script type="text/javascript">
			jQuery(document).ready(function() {
				...
				var vendorName = sting1;
				var vendorName1 = vendorName;
				
				var webSight1 = "${requestScope.Websight}";
				if(webSight1 !== null && webSight1 !== "" && phone !== ""){
					$('#customerName').empty();	
					$('#customerName').append('<a style="color: #CD853F;vertical-align: middle;padding-top: 1px;">'+vendorName1+'</a>');
					var phoneSpilt = phone.split("-");
					$('#phoneCenter').empty();
					$('#phoneCenter').append(" | ");
					$('#phoneName').empty();
					$('#phoneName').append('<label style="color :#CD853F;">'+phoneSpilt[0]+'</label>');
					$('#phone_secondField').empty();
					$('#phone_secondField').append('<label style="color :#CD853F;">'+"-"+phoneSpilt[1]+'</label>');
				}else{
					$('#customerName').empty();	
					$('#customerName').append(vendorName1);
				}
				if(phone !== '' && (webSight1 === null || webSight1 === "")){
					var phoneSpilt = phone.split("-");
					$('#phoneCenter').empty();
					$('#phoneCenter').append(" | ");
					$('#phoneName').empty();
			...
			function hyperLink(){
				var webSight= "${requestScope.Websight}";
				var webSpilt = webSight.split(":");
				///var webPage= "http://"+webSight;
				if(webSpilt[0] === "http"){
					window.open(webSight);
				}else{
					window.open("http://"+webSight);
				}
				//var webSight=${requestScope.Websight};
			}
			...
			{
				if(!$('#vendorFormID').validationEngine('validate')) {
					return false;
				}
				var vendorName = $('#customerNameHeader').val();
				var vendorId = getUrlVars()["rolodexNumber"];
				$.ajax({
					url: "./customerList/updateCustomerName",
					mType: "GET",
					data : { 'customer' : vendorName, 'customerId' :vendorId },
					success: function(data) {
						document.location.href = "./vendordetails?rolodexNumber="+vendorId+"&name="+vendorName+"";
