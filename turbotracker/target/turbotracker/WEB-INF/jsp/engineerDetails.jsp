...
<html>
	<head>
		...
		<title>TurboPro - EngineerDetails</title>
...
<div id="general_journal">
	<div class="tabs_main" style="padding-left: 0px;width:1150px;margin:0 auto; background-color: #FAFAFA">
		<ul>
			<li><a href="rolodex/rolodexcontacts">Contacts</a></li>
			<li><a href="rolodex/rolodexjournal">Journal</a></li>
			<li><a href="rxdetailedviewtabs/engineer">Engineer</a></li>
			...
			<li style="float: right; padding-right: 10px; padding-top:3px;">
				...
				<label id="engineerId" style="display: none;">${requestScope.rolodexNumber}</label>
			</li>
		</ul>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$(".tabs_main").tabs({
			cache: true,
			ajaxOptions: {
				data: {rolodexID: $("#engineerId").text() },
		...
		var enggName = sting1.replace('and', '&');
		var enggName1 = enggName.replace('and', '&');
		var phone = "${requestScope.phone1}";
		$('#customerName').empty();	
		$('#customerName').append(enggName1);
		if(phone !== ''){
	...
	$( "#searchJob" ).autocomplete({ minLength: 3,timeout :1000,
		...
		source: function( request, response ) { var term = request.term;
			...
			lastXhr = $.getJSON( "search/searchEngineerList", request, function( data, status, xhr ) { cache[ term ] = data; 
	...
	{
		if(!$('#engineerFromID').validationEngine('validate')) {
			return false;
		}
		var customerName = $('#customerNameHeader').val();
		var customerId = $('#engineerId').text();
		$.ajax({
