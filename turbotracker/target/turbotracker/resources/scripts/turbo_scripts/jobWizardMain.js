		...
		if(bookedStatus === ''){
			$.ajax({
				...
				success: function (data){
					...
					var aQryStr = "jobNumber="+jobNumber+"&jobName="+jobDescription+"&jobStatus=Booked";
...
function jobMainForm_save() {
	...
	var TaxTerritory = $("#jobMain_TaxTerritory").val();
	var jobMainFormQryStr = "jobCustomer=" + jobCustomer + "&jobDescription=" + jobDescription + "&jobCustomerID=" + jobCustomerId + "&" + data + "&" + jobName_project;
	if(joMasterId === "") {
		$.ajax({
			type:'POST',
			url: "./jobtabs1/createjob",
			data: jobMainFormQryStr,
			success: function(data) {
				var jobNumber = data.jobNumber;
		...
		var updateJobMainQryStr = "joMasterId=" + joMasterId + "&jobDescription=" + jobDescription + "&jobCustomer=" + jobCustomer +"&jobCusromerId=" + jobCustomerId +
												...
												"&enggRXList=" + engineersRXList + "&GcList=" + GCList + "&taxtreeitoty=" + TaxTerritory + "&" + data + "&" + jobName_project;
		$.ajax({
			url: "./job_controller/updatejob",
			type: "POST",
			data : updateJobMainQryStr,
			success: function(data) {
				...
				var aQryStr = "jobNumber=" + jobNumber + "&jobName=" + jobDescription +"&jobStatus=" + jobStatusheaderName;
				document.location.href = "./jobflow?token=view&" + aQryStr;
			}
	   });
	}
			  ...
			  $.ajax({
				...
				success: function(data) {
					...
					jQuery(newDialogDiv).dialog({modal: true, width:300, height:150, title:"Message",
						buttons: [{height:35,text: "OK",click: function() {  $(this).dialog("close");  document.location.href = "./home";}}]}).dialog("open");
					return true;
				}
		  });
		},
