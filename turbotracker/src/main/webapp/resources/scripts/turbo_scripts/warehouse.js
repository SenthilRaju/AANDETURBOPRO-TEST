...
jQuery(document).ready(function() {
	loadWarehouseGrid();
	 editWarehouseGlobalForm = $("#warehouseDetails").serialize();
});
function loadWarehouseGrid() {
	$("#wareHouseGrid").jqGrid({
		...
		colNames : [ 'WareHouse','Description','prWarehouseId','coAccountIdasset','adjustCogAccountID','coTaxTerritoryId','inActive','address1','address2','city','zip','email','isLaser','state','pickticketinfo'],
		colModel : [ 
			...
			{name : 'adjustCogAccountID',index : 'adjustCogAccountID',width : 40,editable : false,hidden : true,editrules : {},editoptions : {}},
			...
			{name : 'city',index : 'city',widadjustCogAccountIDth : 40,editable : false,hidden : true,editrules : {},editoptions : {}},
			...
			{name : 'pickTicketInfo',index : 'pickTicketInfo',width : 40,editable : false,hidden : true,editrules : {},editoptions : {}}
		],
		rowNum : 100,
		pgbuttons : false,
		recordtext : '',
...
function loadDetails(rowId) {
	...
	var cogAccountID = rowData['adjustCogAccountID'];
	...
	var pickTicketInfo=rowData['pickTicketInfo'];
	...
	$('#adjustcog').val(cogAccountID);
	$('#emailPickUp').val(email);
	$('#state').val(state);
	$('#companyName').val(description);
	$('#additionaladdressLine').val(pickTicketInfo);
	if(isActive==='true'){
...
function saveWareHouses(){
	...
	$.ajax({
		...
		success: function(data) {
			editWarehouseGlobalForm = $("#warehouseDetails").serialize();
			createtpusage('Inventory-Warehouses','Inventory Warehouse Save','Info','Inventory,Warhouse Save,description:'+$('#adddescription').val());
			if(data===true)
...
function deleteWareHouses(){
	var WarehouseLocalForm = $("#warehouseDetails").serialize();
	if(editWarehouseGlobalForm != WarehouseLocalForm){
		console.log(editWarehouseGlobalForm+" AVAVA "+WarehouseLocalForm);
		var newDialogDiv = jQuery(document.createElement('div'));
		var errorText ="Warehouse Details modified, please save before leave the page!";
		...
		"Yes": function(){
			jQuery(this).dialog("close");
				$.ajax({
					...
					success: function(data) {
						createtpusage('Inventory-Warehouses','Inventory Warehouse Delete','Info','Inventory,Warhouse Delete,wareHouseID:'+wareHouseID);
						jQuery("#addNewWarehouseDlg").dialog("close");
