...
<div id="vendorGeneral">
	...
	<table>
		<tr height="0px;"></tr>
		<tr>
			<td width="105px;" style="padding-bottom: 10px;"><label>Vendor Name: </label></td>
			 <td width="615px;" style="padding-bottom: 10px;">
				...
				<input type="button" class="savebutton" value="    " onclick="updateVendor()" id="saveButton" >
			</td>
			<td align="right">
				<input type="button" class="quickQuotehoverbutton turbo-blue" value="Address" onclick="callRxAddressDetails()" style="width:90px;"> 
				<input type="button" id="customerQuickQuote" class="quickQuotehoverbutton turbo-blue" disabled value="Quick Quote" onclick="openQuickQuote()" style="width:120px;">
			</td>
		</tr>
		<tr>
		...
		<table>
			...
			<tr>
				<td align="right">
					...
					<input type="button" class="cancelhoverbutton turbo-blue" id="edit"	value=" Edit " style="width:60px" onclick="editRolodexContacts()">
		...
		<table>
			<tr>
				<td align="right"><label>Categories : &nbsp;</label> 
					...
					<input type="checkbox" id="engineerche"	style="vertical-align: middle;display: none;" onclick="engineer()" onChange="updateRolodex()"><!-- <label>&nbsp;Engineer</label> --><label id="Category1lab" style="display: none;"></label>
					<input type="checkbox" id="architectche" style="vertical-align: middle;display: none;" onclick="architect()" onChange="updateRolodex()"><!-- <label>&nbsp;Architect</label> --><label id="Category2lab" style="display: none;"></label>
					<input type="checkbox" id="caustMGR" name="caustMGR" style="vertical-align: middle;display: none;" onChange="updateRolodex()"><!-- <label>&nbsp;G.C/Constr. MGR</label> --><label id="Category3lab" style="display: none;"></label>
					<input type="checkbox" id="owner" name="owner" style="vertical-align: middle;display: none;" onChange="updateRolodex()"><!-- <label>&nbsp;Owner</label> --><label id="Category4lab" style="display: none;"></label>
					<input type="checkbox" id="bondAgent" name="bondAgent" style="vertical-align: middle;display: none;" onChange="updateRolodex()"><!-- <label>&nbsp;Bond Agent</label> --><label id="Category5lab" style="display: none;"></label>
...
<div id="quickQuoteDialog">
	  ...
	  <div id="addProductDialog" align="left">
		...
		<table><tr>
				...
				<td><input type="text" id="TypeDetail" name="TypeDetailName" disabled="disabled" placeholder="Minimum 2 characters required">
			<!-- 	<img alt="search" src="./../resources/scripts/jquery-autocomplete/search.png">  -->
		...
		<table style="align:right;padding-right: 20px;">
			<tr>
	...
	<script type="text/javascript">
		...
		/* 	$("#extensionaddress").hide();
			$("#addressSetting").hide();
			$("#remitaddress").hide(); */
			jQuery("#rxAddressGrid").dialog('option', 'title', 'Vendor Address');
	...
	jQuery(document).ready(function() {
		...
		function openVendorAddress()
		{	
			loadAddressGrid();
			jQuery("#vendorAddressGrid").dialog("open");
		}
		
		jQuery(function(){
			jQuery("#vendorAddressGrid").dialog({
					autoOpen : false,
					modal : true,
		...
		function cancelAddressVendor(){
			$("#vendorAddressGrid").dialog("close");
			return true;
		}
		...
		function editRolodexContacts()
		{
			...
			$("#jobPosition").val(role); $("#email").val(email); 
			$("#division").val(division);
			$("#directLine").val(directline);
		...
		{	
			...
			$("#exchangeId1").val('');$("#subscriberId1").val('');$("#mailAddress").attr("checked", false); $("#remitCheckbox").attr("checked", false); 
			jQuery("#addVendorAddress").dialog("open");
		}

		jQuery(function(){
			jQuery("#addVendorAddress").dialog({
					autoOpen : false,
					modal : true,
		...
		function editAddressVendor()
		{
			...
			var remitto = grid.jqGrid('getCell', rowId, 'isRemitTo');
			...
			if(fax == null || fax.length == 0){
				...
				var exchangecode2 = "";
				if(typeof(areacode2[1])!=="undefined"){
					exchangecode2 = areacode2[1].split("-");
				}
				var pinco = "";
				if(typeof(areacode2[0])!=="undefined"){
					pinco = areacode2[0].split("-");
				}
				var pinco1 = "";
				if(typeof(pinco)!=="undefined"){
			...
			if(fax == null || fax.length == 0){
				...
				if(remitto==='true')
					$("#remitCheckbox").attr('checked', true);
				else
					$('#remitCheckbox').attr("checked", false);
				
				$("#phoneField").show();
				jQuery("#addVendorAddress").dialog("open");
		}

		function deleteAddressVendor(){
			aGlobalVariable = "delete";
			var grid = $("#EmployeeDetailCategoriesGrid");
			var rowId = grid.jqGrid('getGridParam', 'selrow');
			if(rowId === null){
				var errorText = "Please select a Address.";
			...
			var newDialogDiv = jQuery(document.createElement('div'));
			jQuery(newDialogDiv).html('<span><b style="color: red;">Delete the Selected Address?</b></span>');
			jQuery(newDialogDiv).dialog({modal: true, width:300, height:120, title:"Confirm Delete", 
				buttons:{
					"OK": function(){
		...
		function savenewVendor(){
			...
			$.ajax({
				...
				type: "POST",
				async:false,
				data : engineerForm+ "&rolodexNumber="+ rolodexNumber+"&operation=" +aGlobalVariable+"&EmployeeLastName=" +name1,
				success: function(data) {
					
					$("#EmployeeDetailCategoriesGrid").trigger("reloadGrid");
				}
			});jQuery("#addVendorAddress").dialog("close");
			}else if(aGlobalVariable === "edit"){
				var grid = $("#EmployeeDetailCategoriesGrid");
				var rowId = grid.jqGrid('getGridParam', 'selrow');
				var areaCode=$("#areaId").val();
		...
		function loadAddressGrid() {
			...
			$("#EmployeeDetailCategoriesGrid").jqGrid({
				...
				mtype : 'GET',
				colNames : [ '','Address','Address2', 'City', 'State','Zip','Phone','Phone2','Fax','Mailing','Shipping','Remit To' ],
				colModel : 
				[{name : 'rxAddressId',	index : 'rxAddressId',	align : 'left',	width : 100,	editable : false,	hidden : true,	editoptions : {size : 30,readonly : true},editrules : {edithidden : false,required : false}},
				 ...
				 /* {name:'isRemitTo', index:'isRemitTo', align:'center', width:30,hidden : false,editable:false,edittype:"checkbox",formatter:remitToCheckboxFormatter ,editrules:{required:false},editoptions: { value: "true:0"} */
				 {name:'isRemitTo', index:'isRemitTo', align:'center',  width:20, hidden:false, editable:true, formatter:'checkbox', edittype:'checkbox', editrules:{required:false},editoptions: { value: "true:false"}},
				 ],
				recordtext : '',
				rowList : [],
				pgtext : null,
	...
	function remitToCheckboxFormatter(cellValue, options, rowObject){
	//	alert("hi=" +cellValue)
	 	var id="vendorAddressRemitTo_"+options.rowId;
		var element = '';
		if(cellValue)
		     element = "<input type='checkbox' id='"+id+"' name='RemitToName' checked  disabled>";
		else
		   	 element = "<input type='checkbox' id='"+id+"' name='RemitToName' disabled>";
		return element; 
	}
	
	function RemittocheckboxChanges(id){
	...
	$(function() {var cache = {}; var lastXhr=''; $("#customerID").autocomplete({ minLength: 2,timeout :1000,select: function( event, ui ) 
		...
		function viewJobPage(){
			...
			if(rowIdQuote === null){
				...
				var aQryStr = "jobNumber="+aJobNumber+"&jobName="+urijobname+"&jobCustomer="+uricusname+"&jobStatus="+aJobStatus;
