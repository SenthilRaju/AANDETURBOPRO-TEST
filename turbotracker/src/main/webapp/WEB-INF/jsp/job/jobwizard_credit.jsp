...
<table>
					...
					<table>
			            	...
			            	<table>
			            		<tr>
			       					...
			       					<td>
			       						...
			       						<img alt="search" src="./../resources/scripts/jquery-autocomplete/search.png" title="Must be atleast 2 characters long" Onclick="showCustomerDetail(1)"> 
			       					</td>
						...
						<tr><td>
			            	...
			            	<table style="width:385px">
			               		<tr>
			               			<td style="vertical-align:middle;width:120px"><label>
			               			  <c:if test="${requestScope.JobNoticelabel1.valueLong eq 1}">
									    ${requestScope.JobNoticelabel1.valueString}
									  </c:if>
			                	...
			                	<tr>
			                		<td style="vertical-align:middle;width:120px"><label>
									  ...
									  <c:if test="${requestScope.JobNoticelabel2.valueLong eq 1}">
									    ${requestScope.JobNoticelabel2.valueString}
									  </c:if>
			               		...
			               		<tr>
			               			<td style="vertical-align:middle;width:120px"><label>
			               		 	  ...
			               		 	  <c:if test="${requestScope.JobNoticelabel3.valueLong eq 1}">
									    ${requestScope.JobNoticelabel3.valueString}
									  </c:if>
			       				...
			       				<table>
			       					<tr>
			       						...
			       						<td>
			       							...
			       							<img alt="search" src="./../resources/scripts/jquery-autocomplete/search.png" title="Must be atleast 2 characters long" Onclick="showCustomerDetail(2)">
			       						</td>
	...
	<table style="width:900px">
	 	...
	 	<tr>
			<td align="right"><div id="creditMsg" style="display: none"></div></td>
		</tr>
		<tr><td align="right">
	...
	var pretypeValue;
		...
		function parseRequestModelMap() {
			var requestDate = new Date("${requestScope.joMasterDetails.requestedNocdate_Str}");
			var receivedDate = new Date("${requestScope.joMasterDetails.receivedNocdate_Str}");
			var sentNtcDate = new Date("${requestScope.joMasterDetails.sentNtcdate_Str}");
			
			var aRequestedNOCDate = getFormattedDate(requestDate);
			var aReceivedNOCDate = getFormattedDate(receivedDate);
			var aNtcSentGCDate =getFormattedDate(sentNtcDate);

			if(aRequestedNOCDate!='NaN/NaN/NaN')
			$("#credit_requestedNoc").val(aRequestedNOCDate);
			if(aReceivedNOCDate!='NaN/NaN/NaN')
			$("#credit_ReceivedNoc").val(aReceivedNOCDate);
			if(aNtcSentGCDate!='NaN/NaN/NaN')
			...
			var lienWaver = new Date("${requestScope.joMasterDetails.lienWaverSignedDate_Str}");
			var lienThrough = new Date("${requestScope.joMasterDetails.lienWaverThroughDate_Str}");
			
			var waverSighnedDate = getFormattedDate(lienWaver);
			var lienWaverThroughDate = getFormattedDate(lienThrough);

			if(waverSighnedDate!='NaN/NaN/NaN')
			$("#lienWaverSignedDate").val(waverSighnedDate);
			if(lienWaverThroughDate!='NaN/NaN/NaN')
		...
		function credit_save() {
			...
			var creditDataForm = creditFormData +"&jobNumber="+jobNumber+"&joMasterID="+ $("#joMaster_ID").text();
			$.ajax({
				...
				success: function(data) {
					...
					$("#creditMsg").css("display", "block");
					$('#creditMsg').html(validateMsg);
					setTimeout(function(){
					$('#creditMsg').html("");						
					},2000);
					createtpusage('job-Credit Tab','Save Button','Info','Job,Credit Tab,Saving Credit,JobNumber:'+jobNumber);
					return false;
				}
			});
		}
		...
		function RequestedNoc() {
			...
			$("#Credit_requestedNocBY option[value=${requestScope.joMasterDetails.who2}]").attr("selected", true);
			
			if ($('#RequestedNoc_Check').is(':checked')) {
			    if($("#credit_requestedNoc").val().length==0)
				$("#credit_requestedNoc").datepicker("setDate",new Date());
				$(".RequestedNOC").show();
		 	} else {
			    $("#credit_requestedNoc").val("");
				$(".RequestedNOC").hide();
		 	}
		 	return true;
	 	}
		...
		function ReceivedNoc() {
			$("#Credit_ReceivedNocBY option[value=${requestScope.joMasterDetails.who3}]").attr("selected", true);
			
			if ($('#ReceivedNoc_check').is(':checked')) {
				 if($("#credit_ReceivedNoc").val().length==0)
						$("#credit_ReceivedNoc").datepicker("setDate",new Date());
				$(".ReceivedNOC").show();
		 	} else {
		 		 $("#credit_ReceivedNoc").val("");
				$(".ReceivedNOC").hide();
		 	}
	 	}

		function NTCSentGC() {
			$("#Credit_NTCSentGCBY option[value= ${requestScope.joMasterDetails.who4}]").attr("selected", true);
			
			if ($('#NTCSentGC_Check').is(':checked')) {
				if($("#credit_NTCSentGC").val().length==0)
					$("#credit_NTCSentGC").datepicker("setDate",new Date());
				$(".NTCSentGC").show();
		 	} else {
		 		 $("#credit_NTCSentGC").val("");
				$(".NTCSentGC").hide();
		 	}
	 	}
		...
		function lienWaverSignedCheck() {
			$("#lienWaverSignedByList option[value=" +$("#userLogin_Id").val() + "]").attr("selected", true);
			if ($('#lienWaverSigned').is(':checked')) {
				if($("#lienWaverSignedDate").val().length==0)
					$("#lienWaverSignedDate").datepicker("setDate",new Date());
				 $(".lienWaverSigned").show();
				 }
			else {
				 $("#lienWaverSignedDate").val("");
				 $(".lienWaverSigned").hide();
				 }
		}

		function lienWaverThroughCheck() {
			$("#lienWaverThroughByList option[value=" +$("#userLogin_Id").val() + "]").attr("selected", true);
			if ($('#lienWaverThrough').is(':checked')) {
				if($("#lienWaverThroughDate").val().length==0)
					$("#lienWaverThroughDate").datepicker("setDate",new Date());
				 $(".lienWaverThrough").show();
				 }
			else {
				 $("#lienWaverThroughDate").val("");
				 $(".lienWaverThrough").hide();
				 }
		}

		function creditStatusChange1() {
			var creditStatusDate = "${requestScope.joMasterDetails.creditstadate_Str}";
		...
		*Date : 2014-09-17
		*Purpose : Show rolodex customer page on click magnify icon
		*
		*/
		function showCustomerDetail(id){
			...
			var name = '';
			if(id == 1){
				rxNumber = $('#ownerHiddenNameID').val();
				name = $('#ownerNameID').val();
			}else{
				rxNumber = $('#bondHiddenNameID').val();
				name = $('#BondAgentNameID').val();
			}
			if(rxNumber != '' && name != ''){
